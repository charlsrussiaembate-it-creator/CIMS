    ...
    <div className="p-8 max-w-5xl mx-auto">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-1">
          <span className="text-xs font-mono text-[#0ea5e9] bg-[#0ea5e9]/10 px-2 py-0.5 rounded">Staff Portal</span>
        </div>
        <h1 className="text-2xl font-bold text-white">Welcome to CIMS</h1>
        <p className="text-sm text-[#64748b] mt-1">St. Rita's College of Balingasag — Computer Inventory &amp; Maintenance System</p>
      </div>

      {/* Quick stats banner */}
      {(openProblems.length > 0 || needsAttention.length > 0) && (
        <div className="flex gap-3 mb-6 p-4 bg-amber-500/10 border border-amber-500/20 rounded-xl">
      ...
      <div className="bg-[#1e293b] border border-[#334155] rounded-xl p-5">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm font-semibold text-[#94a3b8] uppercase tracking-wider">Recent Problems</h2>
          <button onClick={() => setPage("my-problems")} className="text-xs text-[#0ea5e9] hover:text-[#38bdf8] transition-colors">
            View all →
          </button>
        </div>
          ...
          {recentProblems.map(p => {
            return (
              <div key={p.id} className="flex items-center gap-4 p-3 rounded-lg bg-[#0f172a] border border-[#1e293b]">
                ...
                <div className="text-[10px] font-mono text-[#475569] flex-shrink-0">{p.dateReported}</div>
              </div>
            );
          })}
          {recentProblems.length === 0 && (
            <div className="text-xs text-[#475569] text-center py-4">No problems reported yet</div>
          )}
        </div>
      </div>
      ...
      {needsAttention.length > 0 && (
        <div className="bg-[#1e293b] border border-[#334155] rounded-xl p-5 mt-4">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-semibold text-[#94a3b8] uppercase tracking-wider">Computers Needing Attention</h2>
            <button onClick={() => setPage("computer-status")} className="text-xs text-[#0ea5e9] hover:text-[#38bdf8] transition-colors">
              View all →
            </button>
          </div>
          <div className="space-y-2">
            {needsAttention.map(c => (
              <div key={c.id} className="flex items-center gap-3 p-3 rounded-lg bg-[#0f172a] border border-[#1e293b]">
                <span className="font-mono text-xs text-[#0ea5e9]">{c.id}</span>
                <span className="text-xs text-[#94a3b8] flex-1">{c.location}</span>
